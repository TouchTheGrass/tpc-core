...
import datetime
from tpc.app.services.engine import EngineService
import time
...
class Interaction_With_The_Lobby(WebsocketConsumer):
    ...
    def connect_lobby(self, content ):
        ...
        if GameSession.objects.get(id=session_id).status==GameSessionStatus("game"):
            ...
            if UserGameSession.objects.filter(user_id=user_id, game_session_id=session_id,active=True).exists() == True:
                user_game_session=UserGameSession.objects.filter(user_id=user_id, game_session_id=session_id,active=True)
    ...
    def create_lobby(self, content):
        ...
        if UserGameSession.objects.filter(user_id=user_id, active=True).exists() == False:
    ...
    def move_piece(self, content):
        ...
        if user_game_session.exists() == True and GameSession.objects.filter(id=session_id, status=GameSessionStatus("game")).exists()==True:
